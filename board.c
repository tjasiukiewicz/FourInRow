...
inline static bool try_drop_to_first_row(const struct Token * token_ptr, unsigned column);
/* column normalized to index */
inline static void token_gravity(short column);
/* column normalized to index */
inline static void token_gravity(short column);
...
bool board_insert_token(const struct Token * token, short column) {
    ...
    bool result = true;
    if(! try_drop_to_first_row(token, column)) {
        result = false;
    } else {
        token_gravity(column);
    }
    return result;
}
